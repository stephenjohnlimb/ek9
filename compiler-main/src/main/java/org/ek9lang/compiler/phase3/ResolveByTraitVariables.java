...
final class ResolveByTraitVariables extends TypedSymbolAccess
  ...
  private void resolveAndCheckIdentifier(final EK9Parser.TraitReferenceContext traitCtx,
    ...
    if (symbol instanceof AggregateWithTraitsSymbol trait) {
      ...
      final var variableToResolve = traitCtx.identifier().getText();
      final var resolved = aggregate.resolve(new SymbolSearch(variableToResolve));
  ...
  private void variableIsValidTypeOrError(final ISymbol variable,
                                          final AggregateWithTraitsSymbol trait,
                                          final AggregateWithTraitsSymbol aggregate) {
  ...
  private void emitNotResolved(final String variableToResolve,
                               final AggregateWithTraitsSymbol trait,
                               final AggregateWithTraitsSymbol aggregate) {
