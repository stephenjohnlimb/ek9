...
public class IRInstructionBuilder {
  ...
  private final IRContext originalIRContext;

  /**
   * Create a new instruction builder with access to original IRContext.
   * This enables generators to access IRContext without parameter threading.
   */
  public IRInstructionBuilder(IRGenerationContext context) {
    this.context = context;
    this.memoryManagement = new VariableMemoryManagement();
    this.originalIRContext = context.getIRContext();
  }
  ...
  public LiteralInstr createLiteral(String variableName, String literalValue, String literalType) {
    ...
    context.addInstruction(instruction);
    addMemoryManagement(variableName);
    return instruction;
  }
