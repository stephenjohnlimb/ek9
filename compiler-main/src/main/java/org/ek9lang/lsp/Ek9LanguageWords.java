...
final class Ek9LanguageWords {
  ...
  private void setupKeyWords() {
    ...
    keywordMap.put("as", new KeyWordInformation(
        "MULTI-PURPOSE KEYWORD (context-dependent): (1) Type declaration (variable as Integer), (2) Modifier (as pure, as abstract), (3) HTTP verb mapping (as GET for :/path), (4) Function/class modifier (as function, as class), (5) Trait/application composition syntax. Meaning determined by surrounding context. https://www.ek9.io/basics.html",
        Arrays.asList("as", "as\n", "as open\n", "as abstract\n", "as function\n", "as class\n",
            "as dispatcher\n", "as pure\n", "as pure abstract\n", "as pure function\n",
  ...
  private void populateAccessOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("contains", new KeyWordInformation(
        "CONTAINS: Check if collection/container has specific item (list contains value). Works with List, Dict (checks keys), Set, String (substring). Use for membership testing before access. Returns Boolean: true if item present, false otherwise. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("matches", new KeyWordInformation(
        "MATCHES: Check if value matches pattern or predicate (value matches pattern). Works with String (regex), custom matching logic. Use for pattern validation, regex testing, or custom match rules. Returns Boolean: true if match succeeds, false otherwise. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("empty", new KeyWordInformation(
        "IS EMPTY: Check if collection/container has no elements (list empty). Works with List, Dict, Set, String. Use to test before iteration or to validate non-empty requirement. Returns Boolean: true if no elements, false if has elements. Note: empty collections are still SET (not unset). https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("length", new KeyWordInformation(
        "LENGTH: Get number of elements in collection/container (list length → count). Works with List, Dict, Set, String (character count), Iterator. Use for size checks, iteration bounds, or capacity planning. Returns Integer count of elements. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateConversionOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("$", new KeyWordInformation(
        "STRING CONVERSION: Convert any value to String representation (value$ → \"string\"). All EK9 types implement this operator. Use for display, logging, debugging, or string concatenation. More explicit than implicit toString(). Returns String representation of value. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("$$", new KeyWordInformation(
        "JSON CONVERSION: Convert value to JSON string representation (value$$ → \"{json}\"). Works with records, classes, collections. Use for API responses, data serialization, or persistence. Returns JSON-formatted String. Unset values map to JSON null. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));

    keywordMap.put("#^", new KeyWordInformation(
        "TYPE PROMOTION: Convert value to wider/more general type (Integer → Float, specific type → Any). Returns promoted value if possible, returns unset if promotion not supported. Use when you need automatic type widening for polymorphic operations or generic containers. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("#?", new KeyWordInformation(
        "HASH CODE: Get hash code integer for value (value#? → hash). All EK9 types implement this operator. Use for hash-based collections (Dict, Set), equality comparisons, or custom hashing. Returns Integer hash code. Equal values must have equal hash codes. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("#<", new KeyWordInformation(
        "GET FIRST: Extract first element from collection or sequence (list#< → firstItem). Works with List, String (first character), Iterator. Use when you need just the first element without iteration. Returns first element or unset if collection empty. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("#>", new KeyWordInformation(
        "GET LAST: Extract last element from collection or sequence (list#> → lastItem). Works with List, String (last character). Use when you need just the last element without iteration. Returns last element or unset if collection empty. https://www.ek9.io/operators.html#functional",
        List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populatesInOutOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("->", new KeyWordInformation(
        "INCOMING PARAMETERS: Declare function/method input parameters with types (-> param as Type). Use when: defining function parameters, method arguments, operator inputs. Syntax: `-> param1 as Type1, param2 as Type2`. Parameters are immutable by default. Multiple parameters separated by commas. Use for all function/method/operator parameter declarations. https://www.ek9.io/functions.html",
        List.of(), search -> !search.previousTokenIsPipe()));

    keywordMap.put("<-", new KeyWordInformation(
        "DECLARATION GUARD: Create new variable + check if SET (most common pattern). In control flow: only execute if value is SET. https://www.ek9.io/flowControl.html#guard_operators",
        List.of(), search -> !search.previousTokenIsPipe()));
  ...
  private void populateBooleanOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("and", new KeyWordInformation(
        "DUAL USE (type-dependent): (1) BOOLEAN AND - logical conjunction with short-circuit evaluation (true and false → false), (2) BITWISE AND - binary AND operation on integers (0b1100 and 0b1010 → 0b1000). Operation determined by operand types. https://www.ek9.io/operators.html",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("or", new KeyWordInformation(
        "DUAL USE (type-dependent): (1) BOOLEAN OR - logical disjunction with short-circuit evaluation (true or false → true), (2) BITWISE OR - binary OR operation on integers (0b1100 or 0b1010 → 0b1110). Operation determined by operand types. https://www.ek9.io/operators.html",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("xor", new KeyWordInformation(
        "DUAL USE (type-dependent): (1) BOOLEAN XOR - logical exclusive-or (true xor true → false), (2) BITWISE XOR - binary XOR operation on integers (0b1100 xor 0b1010 → 0b0110). Operation determined by operand types. https://www.ek9.io/operators.html",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("not", new KeyWordInformation(
        "DUAL USE (type-dependent): (1) BOOLEAN NOT - logical negation (not true → false), (2) BITWISE NOT - binary NOT operation (invert all bits). Operation determined by operand type. For bitwise NOT, `~` is preferred. https://www.ek9.io/operators.html",
        List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("~", new KeyWordInformation(
        "THREE DISTINCT USES (context-dependent): (1) BOOLEAN NOT - negate boolean value (~true → false), (2) BITWISE NOT - invert all bits in integer (~0b1010 → 0b0101), (3) REVERSE - reverse order of list/collection items (~[1,2,3] → [3,2,1]). Operation determined by operand type. https://www.ek9.io/operators.html",
        List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateMathematicsOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("+",
        new KeyWordInformation(
            "ADDITION: Add two numbers or concatenate strings (a + b). For numbers: standard arithmetic addition. For strings: concatenation. Use when: numeric calculations, combining values, string building. Returns Integer for integers, Float for floating-point, String for string concatenation. Supports operator overloading for custom types. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("-",
        new KeyWordInformation(
            "SUBTRACTION: Subtract right value from left (a - b) or negate number (-a). Binary form: returns difference. Unary form: returns negative. Use when: numeric calculations, differences, negation. Returns Integer for integers, Float for floating-point. Supports operator overloading for custom types. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("*",
        new KeyWordInformation(
            "MULTIPLICATION: Multiply two numbers (a * b). Standard arithmetic multiplication. Use when: scaling values, calculating products, area/volume calculations. Returns Integer for integers, Float for floating-point. Supports operator overloading for custom types (e.g., matrix multiplication). https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("/",
        new KeyWordInformation(
            "DIVISION: Divide left value by right (a / b). Integer division truncates toward zero. Floating-point division produces decimal results. Use when: splitting values, ratios, averages. Returns Integer for integer operands (truncated), Float for floating-point. Division by zero produces unset value (not exception). https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("+=",
        new KeyWordInformation(
            "ADD ASSIGN: Add value and assign result to variable (a += b → a := a + b). Shorthand for addition assignment. Use when: accumulating totals, incrementing by amount, string building. Mutates variable with sum. For strings, appends to existing value. Returns updated value. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("-=",
        new KeyWordInformation(
            "SUBTRACT ASSIGN: Subtract value and assign result to variable (a -= b → a := a - b). Shorthand for subtraction assignment. Use when: decreasing counters, removing amounts, calculating differences. Mutates variable with difference. Returns updated value. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("*=",
        new KeyWordInformation(
            "MULTIPLY ASSIGN: Multiply value and assign result to variable (a *= b → a := a * b). Shorthand for multiplication assignment. Use when: scaling values, repeated multiplication, compound calculations. Mutates variable with product. Returns updated value. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("/=",
        new KeyWordInformation(
            "DIVIDE ASSIGN: Divide value and assign result to variable (a /= b → a := a / b). Shorthand for division assignment. Use when: scaling down, ratio calculations, averaging. Mutates variable with quotient. Integer division truncates. Returns updated value. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("++",
        new KeyWordInformation(
            "INCREMENT: Add 1 to numeric value returning NEW incremented object (value++ → value+1). EK9 semantics DIFFER from Java/C++: always returns new object (immutable pattern), never mutates in-place. Use for counting, iteration. Returns new Integer/Float with value+1. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("--",
        new KeyWordInformation(
            "DECREMENT: Subtract 1 from numeric value returning NEW decremented object (value-- → value-1). EK9 semantics DIFFER from Java/C++: always returns new object (immutable pattern), never mutates in-place. Use for counting down, iteration. Returns new Integer/Float with value-1. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("^",
        new KeyWordInformation(
            "EXPONENTIATION: Raise number to power (base ^ exponent). Calculates base raised to exponent power. Use when: mathematical power operations, exponential calculations, scientific computing. Example: 2 ^ 8 → 256, 10 ^ 3 → 1000. Returns Integer for integer operands, Float for floating-point. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));

    keywordMap.put("mod",
        new KeyWordInformation(
            "MODULUS: Mathematical modulo operation (a mod b). Always returns non-negative result matching divisor sign. Use for cyclic patterns, array indexing with wrap-around. Different from `rem`: mod always positive, rem matches dividend sign. Returns Integer. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("rem",
        new KeyWordInformation(
            "REMAINDER: Remainder after division (a rem b). Result takes sign of dividend (first operand). Use for splitting values, checking divisibility. Different from `mod`: rem matches dividend sign, mod always positive. Example: -7 rem 3 → -1, -7 mod 3 → 2. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("abs",
        new KeyWordInformation(
            "ABSOLUTE VALUE: Get non-negative magnitude of number (abs of value). Removes sign, returns positive equivalent. Use when: distance calculations, magnitude operations, ensuring non-negative results. Example: abs of -5 → 5, abs of 3 → 3. Returns Integer for Integer input, Float for Float input. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("sqrt",
        new KeyWordInformation(
            "SQUARE ROOT: Calculate square root of number (sqrt of value). Returns value that when multiplied by itself equals input. Use when: distance formulas, geometric calculations, mathematical operations. Example: sqrt of 16 → 4, sqrt of 2 → 1.414... Returns Float for precise decimal results. https://www.ek9.io/operators.html#mathematical",
            List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateEqualityOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("<",
        new KeyWordInformation(
            "LESS THAN: Compare if left value is smaller than right (a < b). Returns Boolean: true if a is less than b, false otherwise. Use when: ordering comparisons, range checks, sorting logic. Works with Integer, Float, String (lexicographic), and types implementing comparison operators. Supports tri-state semantics for optional values. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<?",
        new KeyWordInformation(
            "NULL-SAFE LESS THAN: Compare values but returns unset (not error) if either operand is unset/null. Use when comparing Optional values where unset is valid state. Standard `<` would error on unset operands. Result: true/false if both set, unset if either unset. https://www.ek9.io/operators.html#assignment_coalescing",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<=",
        new KeyWordInformation(
            "LESS THAN OR EQUAL: Compare if left value is smaller than or equal to right (a <= b). Returns Boolean: true if a is less than or equal to b, false otherwise. Use when: range validation, boundary checks, inclusive comparisons. Works with all comparable types. Supports tri-state semantics. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<=?",
        new KeyWordInformation(
    ...
    keywordMap.put(">",
        new KeyWordInformation(
            "DUAL USE (context-dependent): (1) COMPARISON - greater than operator (a > b), (2) STREAM TERMINATOR - add items to collection from stream pipeline (cat items | filter by x > 10 > resultList). Context determines meaning. https://www.ek9.io/operators.html#comparison ",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(">?",
        new KeyWordInformation(
            "NULL-SAFE GREATER THAN: Compare values but returns unset (not error) if either operand is unset/null. Use when comparing Optional values where unset is valid state. Standard `>` would error on unset operands. Result: true/false if both set, unset if either unset. https://www.ek9.io/operators.html#assignment_coalescing",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(">=",
        new KeyWordInformation(
            "GREATER THAN OR EQUAL: Compare if left value is larger than or equal to right (a >= b). Returns Boolean: true if a is greater than or equal to b, false otherwise. Use when: threshold checks, minimum value validation, inclusive range checks. Works with all comparable types. Supports tri-state semantics. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(">=?",
        new KeyWordInformation(
    ...
    keywordMap.put("<=>",
        new KeyWordInformation(
            "THREE-WAY COMPARISON (spaceship operator): Compare two values returning -1 (less), 0 (equal), or 1 (greater). Use for sorting implementations or when you need all three comparison outcomes in one operation (a <=> b). More efficient than separate <, ==, > checks. Returns Integer: -1/0/1. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<~>",
        new KeyWordInformation(
            "FUZZY COMPARISON: Compare values with tolerance for approximate equality. Use for floating-point comparisons (0.1 + 0.2 <~> 0.3 → true), strings with minor differences, or \"close enough\" matching. Returns Boolean indicating if values are approximately equal within tolerance. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<<",
        new KeyWordInformation(
            "BITWISE SHIFT LEFT: Shift bits left in integer value (0b0011 << 2 → 0b1100). Each shift left multiplies by 2. Use for bit manipulation, flag operations, or fast multiplication by powers of 2. Returns Integer with shifted bits. https://www.ek9.io/operators.html",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(">>",
        new KeyWordInformation(
            "DUAL USE (context-dependent): (1) BITWISE - shift bits right on integer (0b1100 >> 2 → 0b0011), (2) STREAM TERMINATOR - append items to collection from stream pipeline (cat items | filter by condition >> existingList). Context determines meaning. https://www.ek9.io/operators.html",
            List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateEqualityAndCoalescingTypeOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("!=",
        new KeyWordInformation(
            "NOT EQUALS: Check if two values are different. `!=` and `<>` are synonyms (work identically). Choose based on preference or codebase consistency. C/Java developers typically prefer `!=`, Pascal/SQL developers prefer `<>`. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("<>",
        new KeyWordInformation(
            "NOT EQUALS: Check if two values are different. `<>` and `!=` are synonyms (work identically). Choose based on preference or codebase consistency. Pascal/SQL developers typically prefer `<>`, C/Java developers prefer `!=`. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("==",
        new KeyWordInformation(
            "EQUALS: Check if two values are equal by value (a == b). Compares object contents, not references (unlike Java). Returns Boolean: true if equal, false if different. Use when: comparing values, equality checks, conditional logic. Works with all EK9 types. Respects tri-state semantics - comparing unset values returns appropriate result. https://www.ek9.io/operators.html#comparison",
            List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateAssignmentTypeOperators(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("=",
        new KeyWordInformation(
            "INITIAL ASSIGNMENT: Used for first-time variable initialization with literal values or simple expressions. For declarations with type safety checking, use `<-` (declaration guard). For updates, use `:=`. Cannot be used in control flow guards. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":",
        new KeyWordInformation(
            "SHORT ASSIGNMENT: Shorthand for `:=` (reassignment). Updates existing variable with new value. Commonly used for simple reassignment. Cannot be used in control flow guards. For safety-checked updates, use `?=` (guarded assignment). https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":=",
        new KeyWordInformation(
            "BLIND ASSIGNMENT: Update existing variable with NO safety check. Use only when you KNOW value is valid. Consider ?= for safety. https://www.ek9.io/operators.html#assignment",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put("::",
        new KeyWordInformation(
            "MODULE REFERENCE: Access constructs from other modules (ModuleName::ConstructName). Scope resolution operator for module-qualified names. Use when: referencing external module constructs, disambiguating names, explicit module imports. Syntax: `com.example.utils::StringHelper`. Common in `references` statements. Enables modular code organization. https://www.ek9.io/basics.html#references_example",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":=:",
        new KeyWordInformation(
            "DEEP COPY: Create complete independent copy of object (target :=: source). Recursively copies all nested objects. Use when you need to modify copy without affecting original. Unlike shallow copy (just references), deep copy duplicates all nested data. Returns new object. https://www.ek9.io/operators.html#modification",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":~:",
        new KeyWordInformation(
            "MERGE: Combine two objects by merging their properties (target :~: source). Source properties override target where they differ. Use for configuration merging, partial updates, or combining data. Non-destructive on source. Returns merged result. https://www.ek9.io/operators.html#modification",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":^:",
        new KeyWordInformation(
            "REPLACE: Replace target object's content with source (target :^: source). Updates target in-place with source values. Use for complete object replacement while preserving target identity/references. Destructive operation on target. Returns modified target. https://www.ek9.io/operators.html#modification",
            List.of(), getSearchNotIndentsAndNotPipe()));
    keywordMap.put(":=?",
        new KeyWordInformation(
            "ASSIGNMENT IF UNSET: Checks LEFT side (current variable) FIRST. Only assigns if currently UNSET. LAZY evaluation (doesn't evaluate if already set). Perfect for caching. NOT same as ?= (opposite check). https://www.ek9.io/flowControl.html#guard_operators",
            List.of(), getSearchNotIndentsAndNotPipe()));
  ...
  private void populateFlowControl(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("if",
        new KeyWordInformation(
            "IF STATEMENT: Conditional execution based on boolean condition. Supports GUARDS for null-safety (if value <- getOptional()). Supports explicit conditions with `with` keyword (if x <- get() with x > 10). No early return - all branches must initialize return variables. Use when: making binary decisions, implementing conditional logic, branching based on conditions. Can combine with guards to eliminate null checks entirely. https://www.ek9.io/flowControl.html#if_elseif_else",
            List.of(), getSearchIndentsOrFirstOrAssignment()));
    keywordMap.put("switch",
        new KeyWordInformation(
            "SWITCH STATEMENT: Multi-way conditional branching with case statements. `switch` and `given` are synonyms (identical behavior). Supports guards for null-safety (switch value <- expression). Use `case` for conditions, `default` for fallback. No fallthrough between cases. https://www.ek9.io/flowControl.html#switch",
            List.of(), getSearchIndentsOrFirstOrAssignment()));
    keywordMap.put("given",
        new KeyWordInformation(
            "GIVEN STATEMENT: Alternative keyword for `switch` - multi-way conditional branching. `given` and `switch` are synonyms (identical behavior). Some developers find `given` reads more naturally. Supports guards and case statements. Choose based on readability preference. https://www.ek9.io/flowControl.html#switch",
            List.of("given"), getSearchIndentsOrFirstOrAssignment()));
    keywordMap.put("case",
        new KeyWordInformation(
            "CASE: Branch condition in switch/given statement. Can match single value (`case 1`), multiple values (`case 1, 2, 3`), ranges, or expressions. NO FALLTHROUGH - each case is independent. Can include guard conditions. Use when: pattern matching specific values, handling specific conditions in switch. Each case executes independently - no break needed (EK9 safety feature). https://www.ek9.io/flowControl.html#switch",
            List.of("case"), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("default",
        new KeyWordInformation(
            "DEFAULT: Fallback case in switch/given when no other case matches. Always executes if no explicit case matched. OPTIONAL but recommended for completeness. Use when: handling unexpected values, ensuring all paths covered. Compiler may require if return value must be initialized in all branches. https://www.ek9.io/flowControl.html#switch",
            List.of("default\n"), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("when",
        new KeyWordInformation(
            "WHEN: Alternative keyword for `if` in conditional expressions. `if` and `when` are synonyms. Some developers find `when` reads more naturally in certain contexts. Use based on readability preference. Identical behavior to `if`. Supports same guard syntax and conditions. https://www.ek9.io/flowControl.html#if_elseif_else",
            List.of("when")));
    keywordMap.put("else",
        new KeyWordInformation(
            "ELSE: Alternative branch when if/when condition is false. Can chain with `else if` for multiple conditions. Final `else` provides default path. Use when: binary decisions need both paths, providing fallback behavior. In functions with return values, compiler may require else to ensure all paths initialize return variable. https://www.ek9.io/flowControl.html#if_elseif_else",
  ...
  private void populateLoops(final Map<String, KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("do",
        new KeyWordInformation(
            "DO-WHILE LOOP: Execute body at least once, then repeat while condition true. Checks condition AFTER body execution. Supports guards in condition (do ... while value <- get()). NO break/continue - use stream pipelines for early termination. Use when: body must execute at least once, polling/retry logic. Condition evaluated after each iteration. https://www.ek9.io/flowControl.html#do_while_loop",
            List.of(), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("while",
        new KeyWordInformation(
            "WHILE LOOP: Repeat body while condition is true. Checks condition BEFORE each iteration. Supports guards in condition (while value <- get()). NO break/continue - use stream pipelines for complex iteration. Use when: unknown iteration count, conditional loops. Body may execute zero times if condition initially false. https://www.ek9.io/flowControl.html#while_loop",
            List.of(), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("for",
        new KeyWordInformation(
            "FOUR DISTINCT USES (context-dependent): (1) LOOP iteration (for item in list), (2) STREAM pipeline filter (cat items | filter for condition), (3) TEXT template language selection (text for language), (4) HTTP SERVICE URI mapping (GET for :/path). Meaning determined by context. https://www.ek9.io/flowControl.html",
            List.of(), getSearchIndentsOrFirstOrAssignment()));
  ...
  private void populateTryCatch(final Map<String, Ek9LanguageWords.KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("try",
        new KeyWordInformation(
            "TRY BLOCK: Begin exception handling block. Code in try block monitored for exceptions. Supports guards for resource management (try resource <- acquire()). Use with catch/handle to handle exceptions, finally for cleanup. Use when: operations may throw exceptions, resource management, error handling. Exceptions propagate to nearest catch block. Guards ensure resources properly initialized. https://www.ek9.io/exceptions.html",
            List.of(), getSearchIndentsOrFirstOrAssignment()));
    keywordMap.put("catch",
        new KeyWordInformation(
            "CATCH EXCEPTION: Handle exceptions in try block. Use `catch -> ex as ExceptionType` to capture exception and handle it. Can have multiple catch blocks for different exception types. `catch` and `handle` are synonyms in EK9. https://www.ek9.io/exceptions.html",
            List.of(), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("handle",
        new KeyWordInformation(
            "HANDLE EXCEPTION: Alternative keyword for `catch` - handles exceptions in try block. Use `handle -> ex as ExceptionType` to capture exception. `handle` and `catch` are synonyms (identical behavior). Choose based on readability preference. https://www.ek9.io/exceptions.html",
            List.of(), TokenResult::previousTokensIndentsOrFirst));
    keywordMap.put("finally",
        new KeyWordInformation(
            "FINALLY BLOCK: Cleanup code that ALWAYS executes after try/catch blocks, regardless of whether exception occurred or was caught. Executes even if try/catch contains return statement. Use for guaranteed cleanup operations: closing files, releasing resources, logging, resetting state. Works with guards for resource safety. Use when: cleanup must happen no matter what, resource release is critical, state must be restored. Finally blocks cannot be skipped - guaranteed execution. https://www.ek9.io/exceptions.html",
            List.of(), TokenResult::previousTokensIndentsOrFirst));
  ...
  private void populateStreaming(final Map<String, Ek9LanguageWords.KeyWordInformation> keywordMap) {
    ...
    keywordMap.put("cat", new KeyWordInformation(
        "CATENATE STREAM: Start streaming pipeline from collection/source (cat items | filter | map). Short for concatenate - initiates stream processing. Use to begin any stream pipeline. Can concatenate multiple sources (cat list1, list2). Returns Stream for pipeline operations. https://www.ek9.io/streamsAndPipelines.html",
        List.of(), getSearchIndentsOrFirstOrAssignment()));

    keywordMap.put("|", new KeyWordInformation(
        "PIPE OPERATOR: Link stream operations into processing pipeline (cat items | filter | map | collect). Chains operations left-to-right, passing results from one to next. Each operation receives stream from previous operation. Enables declarative data transformation. Use to compose stream operations. Fundamental to EK9 streaming. Similar to Unix pipe - clean, readable data flow. https://www.ek9.io/streamsAndPipelines.html",
        List.of(), getSearchNotIndentsAndNotPipe()));

    keywordMap.put("filter", new KeyWordInformation(
        "FILTER STREAM: Keep only items that match predicate in stream pipeline. `filter` and `select` are synonyms (identical behavior). Use `cat items | filter by condition` or `filter with predicate`. Choose based on readability preference - `filter` is more common. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("filter", "filter with", "filter by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("select", new KeyWordInformation(
        "SELECT STREAM: Alternative keyword for `filter` - keeps only items that match predicate. `select` and `filter` are synonyms (identical behavior). Use `cat items | select by condition`. SQL developers may find `select` more familiar. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("select", "select with", "select by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("map", new KeyWordInformation(
        "MAP STREAM: Transform each stream item to different value or type using function (cat items | map with transformer). Applies transformation function to every item. Use when: converting types, extracting properties, applying calculations. Use `map with function` or `map by property`. Returns stream of transformed items. Common pattern for data transformation pipelines. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("map", "map with", "map by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("sort", new KeyWordInformation(
        "SORT STREAM: Order stream items using comparator function or natural ordering (cat items | sort by comparator). Sorts all items in stream. Use when: need ordered output, ranking items, organizing data. Use `sort with comparator` for custom ordering, `sort by property` for property-based sorting. Returns stream of sorted items. Maintains stable sort order. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("sort", "sort with", "sort by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("group", new KeyWordInformation(
        "GROUP STREAM: Collect stream items into groups based on key property or function (cat items | group by category). Creates map of key to list of items with that key. Use when: categorizing data, aggregating by property, creating lookups. Use `group by property` or `group with function`. Returns Dict mapping keys to grouped items. Common for aggregation and reporting. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("group", "group with", "group by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("join", new KeyWordInformation(
        "JOIN STREAMS: Combine items from stream into single string or merge streams. Use `join with separator` for string concatenation, or join multiple streams together. Use for creating delimited output, combining data sources. Returns joined result. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("join", "join with", "join by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("split", new KeyWordInformation(
        "SPLIT STREAM: Divide stream items based on property or predicate. Use `split by criteria` to partition stream into sub-streams. Use for categorization, parallel processing paths, or conditional routing. Returns split streams. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("split", "split with", "split by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("uniq", new KeyWordInformation(
        "UNIQUE STREAM: Remove duplicate items from stream, keeping only first occurrence (cat items | uniq). Filters stream to distinct values only. Use when: eliminating duplicates, finding unique values, deduplication. Use `uniq by property` to determine uniqueness by specific property. Returns stream of unique items. Preserves first occurrence order. Common for data cleaning. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("uniq", "uniq with", "uniq by"), TokenResult::previousTokenIsPipe));
    keywordMap.put("tee", new KeyWordInformation(
        "TEE STREAM: Split stream sending copies to multiple outputs while continuing main pipeline (like Unix tee command). Use `tee with consumer` to perform side effects (logging, metrics) without interrupting stream flow. Use for debugging, monitoring, or parallel processing. Returns original stream. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("tee", "tee with", "tee by", "tee in"), TokenResult::previousTokenIsPipe));
    keywordMap.put("flatten", new KeyWordInformation(
        "FLATTEN STREAM: Extract items from nested collections into single flat stream (cat listOfLists | flatten). Unnests one level of collection hierarchy. Use when: working with nested structures, combining multiple collections, converting 2D to 1D. Flattens List of Lists into single List. Returns flat stream of all nested items. Common for processing hierarchical data. https://www.ek9.io/streamsAndPipelines.html",
        List.of("flatten"), TokenResult::previousTokenIsPipe));
    keywordMap.put("call", new KeyWordInformation(
        "CALL FUNCTION: Invoke function/method on each stream item synchronously (cat items | call with processor). Blocks until each call completes. Use for sequential processing with functions. Each item processed in order. Returns stream of results. https://www.ek9.io/streamsAndPipelines.html",
        List.of("call"), TokenResult::previousTokenIsPipe));
    keywordMap.put("async", new KeyWordInformation(
        "ASYNC CALL: Invoke function/method on stream items asynchronously without blocking (cat items | async with processor). Processes items concurrently. Use for I/O-bound operations, parallel processing, or when order doesn't matter. Returns stream of Future results. https://www.ek9.io/streamsAndPipelines.html",
        List.of("async"), TokenResult::previousTokenIsPipe));
    keywordMap.put("skip", new KeyWordInformation(
        "SKIP STREAM: Discard first N items from stream, passing rest through (cat items | skip 5). Removes items from beginning of stream. Use when: ignoring headers, pagination (skip offset), removing leading items. Use `skip N` to skip fixed count. Returns stream without first N items. Combines with head for pagination. Common for data windowing. https://www.ek9.io/streamsAndPipelines.html",
    ...
    keywordMap.put("tail", new KeyWordInformation(
        "TAIL STREAM: Take only last N items from stream, discarding earlier ones (cat items | tail 10). Keeps trailing items only. Use when: getting recent items, last page of results, monitoring latest events. Use `tail N` to take last N items, `tail` alone takes last item. Returns stream of at most N items from end. Must process entire stream to find last N items. https://www.ek9.io/streamsAndPipelines.html",
        Arrays.asList("tail", "tail by", "tail of", "tail only"),
        TokenResult::previousTokenIsPipe));
    keywordMap.put("collect", new KeyWordInformation(
        "COLLECT STREAM: Terminate stream pipeline and gather all items into collection (cat items | filter | map | collect). Materializes stream results. Use when: need concrete collection, storing results, ending pipeline. Use `collect` for List, `collect as Type` for specific collection type. Returns final collection of all streamed items. Terminal operation - ends stream processing. Required to get actual results from pipeline. https://www.ek9.io/streamsAndPipelines.html",
