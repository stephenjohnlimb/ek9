#!/usr/bin/env bash
#
# E2E test for isSetOperator
# Compiles and executes with -Cg flag (validates @BYTECODE with SMAP)
#

set -e
cd "$(dirname "$0")"

# Project-relative paths (8 levels up to project root from isSetOperator/)
...
# Compile with debug instrumentation (-Cg validates @BYTECODE with SourceDebugExtension)
export EK9_HOME
$EK9_BINARY -Cg *.ek9

# Execute using ek9 binary
$EK9_BINARY isSetOperator.ek9 > actual_output.txt 2>&1
