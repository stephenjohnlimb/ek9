#!/usr/bin/env bash
#
# E2E test for tryWithResourceFinally
# Tests try-with-resource + finally statement (no catch)
#
...
$EK9_BINARY tryWithResourceFinally.ek9 > actual_output.txt 2>&1
if ! diff -u expected_output.txt actual_output.txt; then
    echo "FAIL: output mismatch"
    exit 1
fi

echo "PASS: tryWithResourceFinally"
