#!/usr/bin/env bash
#
# E2E test for forInWithGuard
# Multi-case test: Tests for-in loop with guard variable pattern
#
...
# Test case 1: with items (guard set, iterates 3 items)
$EK9_BINARY forInWithGuard.ek9 1 > actual_case_1.txt 2>&1
if ! diff -u expected_case_1.txt actual_case_1.txt; then
    echo "FAIL: Case 1 (items provided) mismatch"
    exit 1
fi

# Test case 2: no items (guard unset, no iterations)
$EK9_BINARY forInWithGuard.ek9 0 > actual_case_0.txt 2>&1
if ! diff -u expected_case_0.txt actual_case_0.txt; then
    echo "FAIL: Case 0 (guard unset) mismatch"
    exit 1
fi

echo "PASS: forInWithGuard (2 cases)"
