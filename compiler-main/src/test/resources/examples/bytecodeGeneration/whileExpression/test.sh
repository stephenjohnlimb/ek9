#!/usr/bin/env bash
#
# E2E test for whileExpression
# Tests while expression accumulator pattern with different limit values
#
...
$EK9_BINARY whileExpression.ek9 5 > actual_limit_5.txt 2>&1
...
$EK9_BINARY whileExpression.ek9 3 > actual_limit_3.txt 2>&1
...
$EK9_BINARY whileExpression.ek9 0 > actual_limit_0.txt 2>&1
if ! diff -u expected_limit_0.txt actual_limit_0.txt; then
    echo "FAIL: limit=0 mismatch"
    exit 1
fi

echo "PASS: whileExpression (3 cases)"
