#!/usr/bin/env bash
#
# E2E test for simpleSwitchWithGuard
# Multi-case test: Tests switch statement with guard variable pattern
#
...
# Test case 1: value=1 (matches case 1)
$EK9_BINARY simpleSwitchWithGuard.ek9 1 > actual_case_1.txt 2>&1
if ! diff -u expected_case_1.txt actual_case_1.txt; then
    echo "FAIL: Case 1 mismatch"
    exit 1
fi

# Test case 2: value=0 (guard unset, switch skipped)
$EK9_BINARY simpleSwitchWithGuard.ek9 0 > actual_case_0.txt 2>&1
if ! diff -u expected_case_0.txt actual_case_0.txt; then
    echo "FAIL: Case 0 (guard unset) mismatch"
    exit 1
fi

echo "PASS: simpleSwitchWithGuard (2 cases)"
