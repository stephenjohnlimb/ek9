...
### Capturing Bytecode with showBytecode Flag

**RECOMMENDED WORKFLOW**: Use the `showBytecode` flag in the test constructor to capture normalized bytecode output directly, rather than manually constructing expected bytecode.
...
```java
AbstractBytecodeGenerationTest(
    String fromResourcesDirectory,
    List<SymbolCountCheck> expectedSymbols,
    boolean verbose,           // Show parse tree
    boolean muteReportedErrors,// Mute error output
    boolean showBytecode       // Output generated bytecode to stdout
)
```
