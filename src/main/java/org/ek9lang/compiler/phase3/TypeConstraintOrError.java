 ...
 * While superficially simple looking, this does use recursion to revolve the types.
 */
final class TypeConstraintOrError extends TypedSymbolAccess
    implements BiConsumer<AggregateSymbol, EK9Parser.ConstrainDeclarationContext> {
  private final SymbolFactory symbolFactory;

  TypeConstraintOrError(final SymbolsAndScopes symbolsAndScopes,
  ...
  public void accept(final AggregateSymbol aggregateSymbol,
    ...
    final var possibleExpression = constrainTypeOrError(aggregateSymbol, ctx.constrainType());
   ...
   */
  private Optional<ISymbol> getConstrainTypeOrError(final AggregateSymbol aggregateSymbol,
                                                    final EK9Parser.ConstrainTypeContext ctx) {

    if (ctx.literal() != null) {
      return literalConstrainTypeOrError(aggregateSymbol, ctx);
    }

    if (ctx.constrainType().size() == 1) {
      return constrainTypeOrError(aggregateSymbol, ctx.constrainType(0));
    }

    return constrainTypeOrError(aggregateSymbol, ctx.constrainType(0), new Ek9Token(ctx.op), ctx.constrainType(1));
  }
   ...
   * </pre>
   */
  private Optional<ISymbol> constrainTypeOrError(final AggregateSymbol aggregateSymbol,
                                                 final EK9Parser.ConstrainTypeContext ctx) {

    //At this point we do not expect any expression to have been recorded against this node.
    final Optional<ISymbol> rtn = getConstrainTypeOrError(aggregateSymbol, ctx);
  ...
  private Optional<ISymbol> constrainTypeOrError(final AggregateSymbol aggregateSymbol,
    ...
    final var first = constrainTypeOrError(aggregateSymbol, ctx1);
    final var second = constrainTypeOrError(aggregateSymbol, ctx2);
  ...
  private Optional<ISymbol> literalConstrainTypeOrError(final AggregateSymbol aggregateSymbol,
                                                        final EK9Parser.ConstrainTypeContext ctx) {
