...
public abstract class ResolutionDirectiveListener implements CompilationPhaseListener {
  ...
  protected abstract void symbolMatch(final ErrorListener errorListener, final ResolutionDirective directive,
                                      final ISymbol symbol);

  protected abstract void noSymbolMatch(final ErrorListener errorListener, final ResolutionDirective directive);
  ...
  protected void processDirectives(final CompilationEvent compilationEvent, List<Directive> directives) {
    ...
    for (var directive : directives) {
      ...
      try {
        ...
        resolved.ifPresentOrElse(symbol -> symbolMatch(errorListener, resolutionDirective, symbol),
            () -> noSymbolMatch(errorListener, resolutionDirective)
        );
