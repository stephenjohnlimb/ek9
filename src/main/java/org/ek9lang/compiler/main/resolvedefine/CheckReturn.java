...
public class CheckReturn extends RuleSupport implements BiConsumer<ISymbol, ISymbol> {
  ...
  public void accept(final ISymbol parentSymbol, final ISymbol returnSymbol) {
    ...
    if (isNotAbstractAndIsTyped(parentSymbol, returnSymbol)
        && isNotExternOrVoidType(parentSymbol, returnSymbol)) {
  ...
  private boolean isNotExternOrVoidType(final ISymbol parentSymbol, final ISymbol returnSymbol) {
    var isExternallyImplemented = "TRUE".equals(parentSymbol.getSquirrelledData(EXTERN));
    var isVoidReturnType = EK9_VOID.equals(returnSymbol.getType().get().getFullyQualifiedName());
    return !isExternallyImplemented && !isVoidReturnType;
  }
