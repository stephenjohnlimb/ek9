...
public class ErrorListener extends BaseErrorListener
{
	...
	public void semanticError(Token offendingToken, String msg, SemanticClassification classification, MatchResults results)
	{
	...
	public void semanticWarning(Token offendingToken, String msg, SemanticClassification classification)
	{
		...
		else
		{
			...
			warning = new ErrorDetails(ErrorClassification.SEMANTIC_WARNING, offendingToken.getText(), null, offendingToken.getLine(), offendingToken.getCharPositionInLine(), tokenLength, msg);
		}
	...
	private ErrorDetails createSemanticError(Token offendingToken, String msg, SemanticClassification classification)
	{
		...
		else
		{
			...
			int tokenLength = offendingToken.getText().length();
			error = new ErrorDetails(ErrorClassification.SEMANTIC_ERROR, offendingToken.getText(), shortFileName, offendingToken.getLine(), offendingToken.getCharPositionInLine(), tokenLength, msg);
		}
	...
	public enum SemanticClassification
	{
		...
		GENERIC_TYPE_OR_FUNCTION_PARAMETERS_INCORRECT("type/function is generic but incorrect number of parameters supplied"),
		GENERIC_TYPE_CONSTRUCTOR_INAPPROPRIATE("type is generic, but for type inference to work; the number of generic and constructor parameters must be the same"),
		...
		METHOD_ACCESS_MODIFIER_DEFAULT("access modifier is not really needed here - all class methods are 'public' by default"),
		...
		GENERIC_FUNCTION_CANNOT_BE_ABSTRACT("implementation must be provided for generic functions, but can be overridden when used"),
		...
		INCOMPATIBLE_TYPES_BUT_CONSTRUCTOR_EXISTS("- constructor would be appropriate: direct type usage is not appropriate in this context"),
		...
		IMPLIED_OPERATOR_NOT_DEFINED_FROM_GENERIC("implied operator must be defined when using a generic implementation with a specific type"),
	...
	public static class ErrorDetails extends Details
	{
		...
		private ErrorDetails(ErrorClassification classification, String likelyOffendingSymbol, String shortFileName, int lineNumber, int characterNumber, int tokenLength, String typeOfError)
		{
	...
	public static abstract class Details
	{
		...
		private Details(String likelyOffendingSymbol, String shortFileName, int lineNumber, int characterNumber, int tokenLength, String typeOfError)
		{
