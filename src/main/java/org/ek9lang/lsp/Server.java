...
public final class Server {
  ...
  public static void main(final String[] args) throws ExecutionException, InterruptedException {
    ...
    final var startListening = runEk9LanguageServer(new OsSupport(), System.in, System.out,
        true, true);
   ...
   */
  public static Future<Void> runEk9LanguageServer(final OsSupport osSupport,
    ...
    final var globalLogger = Logger.getLogger(Logger.GLOBAL_LOGGER_NAME);
    globalLogger.setLevel(Level.OFF);

    //Ready for document and workspace processing.

    final var languageServer = new Ek9LanguageServer(osSupport);
    languageServer.getCompilerConfig().setProvideLanguageHoverHelp(provideLanguageHoverHelp);

    final var launcher = LSPLauncher.createServerLauncher(languageServer, in, out);
    final var client = launcher.getRemoteProxy();
