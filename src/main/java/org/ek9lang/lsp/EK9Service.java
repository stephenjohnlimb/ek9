...
public abstract class EK9Service
{
	...
	private ErrorsToDiagnostics diagnosticExtractor = new ErrorsToDiagnostics();
	public EK9Service()
	{
	...
	protected void reportOnCompiledSource(CompilableSource compilableSource)
	{
		...
		if(compilableSource != null)
		{
			...
			sendDiagnotics(sourceDiagnostics);
		}		
	}
	...
	protected void clearOldCompiledDiagnostics(CompilableSource compilableSource)
	{
		if(compilableSource != null)
		{
			PublishDiagnosticsParams clearedDiagnotics = diagnosticExtractor.getEmptyDiagnostics(compilableSource);
			sendDiagnotics(clearedDiagnotics);
		}
	}
	
	/**
	 * This is how to send compiler errors back.
	 * @param diagnotics The set of diagnostics to be returned to the user.
	 */
	public void sendDiagnotics(PublishDiagnosticsParams diagnotics)
	{
		getLanguageServer().getClient().publishDiagnostics(diagnotics);
	}
