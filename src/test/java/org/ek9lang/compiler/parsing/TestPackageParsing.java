...
public class TestPackageParsing
{
	 ...
	 */
	private final Function<String, EK9SourceVisitor> validParsedFile = resourceName -> Optional.of(resourceName)
			.stream()
			.map(fileForClassPathResource)
			.map(validFileParser)
			.findFirst().orElseThrow(() -> new RuntimeException("Expecting to parse file " + resourceName));
	...
	private final Consumer<PackageDetails> validateSmallPackage = details -> assertEquals("ekopen.net.handy.tools", details.moduleName());
	...
	public void testPackaging()
	{
		...
		toTest.entrySet().parallelStream().forEach(entry -> processTest(entry.getKey(), entry.getValue()));
	}
